...
second:
	lb $k0,9($a0)                  #Increase the  seconds up to 9
	addi $k0,$k0,1                 #For number more than 9,go to 
	bgt $k0,$s0,tensecond          #tensecond
	sb $k0,9($a0)
	j retu

tensecond:
	lb $k0,8($a0)                  #Increase the seconds up to 59
	addi $k0,$k0,1                 #If more than 59 seconds,go to 
	bgt $k0,$s1,minute             #minute
	sb $s2,9($a0)
	sb $k0,8($a0)
	j retu

minute:
	lb $k0,6($a0)                #Increase minutes up to 9
	addi $k0,$k0,1               #For more than 9 minutes
	bgt $k0,$s0,tenminute        #Go to tenminute
	sb $s2,9($a0)
	sb $s2,8($a0)
	sb $k0,6($a0)
	j retu 
...
tenminute:
	lb $k0,5($a0)                #Increase minutes up to 99
	addi $k0,$k0,1               #For number more than 99
	bgt $k0,$s0,ret              #just dont change the clock 
	sb $s2,9($a0)                #anymore
	sb $s2,8($a0)
	sb $s2,6($a0)
	sb $k0,5($a0)
...
# after all interruption handle done
...
__start:
	...
	.data
time:	.byte 8,8,8,8,8,48,48,58,48,48,0   #string buffer set in ASCII dec code
...
main:
	...
	li $t0,0x00008801  #Enable interrupt for keyboard and timer through
	mtc0 $t0,$12       #Status register
	
	addi $t0,$zero,100  #Set time compare to be 100
	mtc0 $t0,$11
	mtc0 $zero,$9

	
	move $t1,$zero  #set t1 as the change indicator
	la $a0,time     #printer the time
	addi $a0,$a0,5
	jal display
	
forever:
	beqz $t1,forever   #create an infinite loop 
	move $t1,$zero     #to keep printing the string 
	la $a0,time
	jal display
	j forever

#This function is used to display the string char by char
display:
	lb $s1,0($a0) #store the character into s1   #Check whether its the end
	beqz $s1,done                                #of the string

poll: 
	lw $s2,0xffff0008  #Set s2 to read from display control register
	andi $s2,$s2,0x01  #loop until the display is ready for output
	beqz $s2,poll
	...
	jr $ra       #jump back to return register
